...
final class RegexHound {
    ...
    static final List<Rule> RULES = List.of(
            ...
            // LLM providers
            r("openai-api-key", "OpenAI API key", "openai", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\bsk-[A-Za-z0-9]{20}T3BlbkFJ[A-Za-z0-9]{20}\\b")),
            r("openai-project-key", "OpenAI project/service key", "openai", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\bsk-(?:proj|svcacct|admin)-[A-Za-z0-9_-]{20,255}\\b")),
            r("anthropic-api-key", "Anthropic API key", "anthropic", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\bsk-ant-(?:api|admin)[0-9]{2}-[A-Za-z0-9_-]{80,120}\\b")),

            // Modern SaaS / dev-infra (distinctive prefixes, self-validating)
            r("square-access-token", "Square access token", "square", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\b(?:sq0atp|sq0csp)-[A-Za-z0-9_-]{22,43}\\b")),
            r("square-oauth-token", "Square OAuth token", "square", "secret", Severity.CRITICAL, Confidence.MEDIUM,
                    cs("\\bEAAA[A-Za-z0-9_-]{60}\\b")),
            r("sentry-dsn", "Sentry DSN with embedded key", "sentry", "secret", Severity.HIGH, Confidence.HIGH,
                    ci("\\bhttps://[0-9a-f]{32}@[A-Za-z0-9.-]+\\.ingest\\.(?:[a-z]{2}\\.)?sentry\\.io/\\d+")),
            r("postman-api-key", "Postman API key", "postman", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\bPMAK-[a-f0-9]{24}-[a-f0-9]{34}\\b")),
            r("figma-token", "Figma personal access token", "figma", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\bfigd_[A-Za-z0-9_-]{40,255}\\b")),
            r("grafana-service-account-token", "Grafana service-account token", "grafana", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\bglsa_[A-Za-z0-9]{32}_[A-Fa-f0-9]{8}\\b")),
            r("grafana-cloud-token", "Grafana cloud access-policy token", "grafana", "secret", Severity.CRITICAL, Confidence.MEDIUM,
                    cs("\\bglc_[A-Za-z0-9+/]{32,}={0,2}\\b")),
            r("notion-token", "Notion integration token", "notion", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\b(?:secret_|ntn_)[A-Za-z0-9]{40,64}\\b")),
            r("linear-api-key", "Linear API key", "linear", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\blin_api_[A-Za-z0-9]{40,64}\\b")),
            r("airtable-pat", "Airtable personal access token", "airtable", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\bpat[A-Za-z0-9]{14}\\.[A-Za-z0-9]{64}\\b")),
            r("databricks-token", "Databricks personal access token", "databricks", "secret", Severity.CRITICAL, Confidence.MEDIUM,
                    cs("\\bdapi[a-f0-9]{32}\\b")),
            r("doppler-token", "Doppler token", "doppler", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\bdp\\.(?:pt|st|ct|sa|scim|audit)\\.[A-Za-z0-9]{40,44}\\b")),
            r("vault-service-token", "HashiCorp Vault service token", "hashicorp", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\bhvs\\.[A-Za-z0-9_-]{90,120}\\b")),
            r("mapbox-secret-token", "Mapbox secret token", "mapbox", "secret", Severity.HIGH, Confidence.MEDIUM,
                    cs("\\bsk\\.eyJ[A-Za-z0-9_-]{50,}\\.[A-Za-z0-9_-]{20,}\\b")),

            // Family completion for providers already partially covered
            r("stripe-webhook-secret", "Stripe webhook signing secret", "stripe", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\bwhsec_[A-Za-z0-9]{32,64}\\b")),
            r("stripe-test-key", "Stripe test secret/restricted key", "stripe", "secret", Severity.HIGH, Confidence.HIGH,
                    cs("\\b(?:sk|rk)_test_[A-Za-z0-9]{16,255}\\b")),
            r("gitlab-token-family", "GitLab typed token", "gitlab", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\bgl(?:dt|rt|cbt|ffct|imt|ptt|oas|soat|agent)-[A-Za-z0-9_-]{16,255}\\b")),
            r("slack-app-token", "Slack app-level token", "slack", "secret", Severity.CRITICAL, Confidence.HIGH,
                    cs("\\bxapp-\\d-[A-Za-z0-9]{8,}-\\d+-[A-Za-z0-9]{32,}\\b")),

            // Composite credentials
            r("npmrc-auth-token", "npmrc _authToken", "npm", "secret", Severity.CRITICAL, Confidence.MEDIUM,
                    ci("^//[\\w.:/-]+/:_authToken\\s*=\\s*[\\\"']?([A-Za-z0-9_./=+-]{20,})"), 1, 3.0),
            r("gcp-service-account-json", "GCP service-account key JSON", "google", "private-key", Severity.CRITICAL, Confidence.HIGH,
                    dotall("[\\\"']type[\\\"']\\s*:\\s*[\\\"']service_account[\\\"'][\\s\\S]{0,400}?[\\\"']private_key[\\\"']")),
            r("firebase-fcm-server-key", "Firebase Cloud Messaging server key", "google", "secret", Severity.CRITICAL, Confidence.MEDIUM,
                    cs("\\bAAAA[A-Za-z0-9_-]{7}:[A-Za-z0-9_-]{140}\\b")),
